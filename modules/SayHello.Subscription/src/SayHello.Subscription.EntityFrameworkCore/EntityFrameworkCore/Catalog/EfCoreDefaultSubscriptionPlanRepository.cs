...
public class EfCoreDefaultSubscriptionPlanRepository : IDefaultSubscriptionPlanRepository, ITransientDependency
{
    ...
    public EfCoreDefaultSubscriptionPlanRepository(IDbContextProvider<ISubscriptionDbContext> provider)
    {
        _provider = provider;
    }

    public virtual async Task<DefaultSubscriptionPlan?> FindAsync(string productCode,
        CancellationToken cancellationToken = default)
    {
        ...
        var selected = await (
            ...
            where product.Code == productCode && product.DefaultPlanId != null
            from plan in db.SubscriptionPlans.AsNoTracking().Include(x => x.Entitlements)
                .Where(x => x.ProductId == product.Id && x.Id == product.DefaultPlanId)
                .DefaultIfEmpty()
        ...
        DateTime now, CancellationToken cancellationToken = default)
    {
        ...
        query.Validate();
        var db = await _provider.GetDbContextAsync();
        var products = db.SubscriptionProducts.Where(x =>
        ...
        products = products.Where(product => !db.UserSubscriptions.Any(subscription =>
            subscription.UserId == userId && subscription.ProductId == product.Id &&
            subscription.IsCurrent && subscription.EndedAt == null &&
            subscription.StartsAt <= now && (subscription.ExpiresAt == null || subscription.ExpiresAt > now)));
        var plans = db.SubscriptionPlans.Where(plan => plan.State == SubscriptionCatalogState.Published &&
        ...
        var items = await (
            ...
            join product in db.SubscriptionProducts.AsNoTracking() on plan.ProductId equals product.Id
